...
describe("tree traversal", () => {
  ...
  it("very basic traversal", () => {
    let tree = {
      tag: 'div', props: { id: 1 }, children: [
        { tag: 'span', props: { id: 2 }, content: 'hello' },
        { tag: 'span', props: { id: 3 }, content: 'world' }
      ]
    };
    ...
    const basicRender = (tagGetter, propGetter, props, children, node) => {
      // console.log('rendering..', JSON.stringify(node))
      numRenderCalls++;
      if (isString(node)) {
        // console.log("Leaf: ", node, propGetter(node))
        return propGetter(node);
      }
      const tag = tagGetter(node);

      // 'm' prefix for mapped or modified
      return { tag, mprops: props, mchildren: children };
    };

    const treeWalker = traverse(basicRender, tagGetter, propGetter);
    const results = treeWalker(tree);

    // console.log(JSON.stringify(results, null, 2), numRenderCalls);

    expect(numRenderCalls).to.equal(5);
    expect(results.tag).to.equal('div');
    expect(results.mprops.id).to.equal(1);
    expect(results.mchildren[0].mprops.id).to.equal(2);
    expect(results.mchildren[0].mchildren[0]).to.equal('hello');
    expect(results.mchildren[1].mprops.id).to.equal(3);
    expect(results.mchildren[1].mchildren[0]).to.equal('world');
