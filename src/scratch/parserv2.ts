...
const makeParser = (indentLevel: number): P.Language =>
    ...
    P.createLanguage({
        ...
        tag: r => {
            ...
            return P.seq(
                r.ident.skip(spaces).map((tag: any) => ({ tag })),
                oneOf(r.attrList)
                    .map((attrList) => ({ attrList: attrList === "" ? [] : attrList })),
                oneOf(
                    r.content.map((content: any) => ({ content })),
                    childrenParser.map((children: any) => ({ children })),
                    spaces,
                ),
            ).map((val: any[]) => ({ ...val[0], ...val[1], ...val[2] }));
        },
        ...
        //  JSON value.
        value: r =>
            oneOf(r.object, r.array, r.string, r.number, r.null, r.true, r.false)
...
function prettyPrint(x: any) {
    let opts: util.InspectOptions = { depth: null, colors: true };
