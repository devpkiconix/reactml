...
const mappedPropId = P.alt(
    $p.then(ident.sepBy(dot))
        .map(a => a.join("."))
        .map(propName => ({ type: "$p", id: propName })),
    $g.then(ident.sepBy(dot))
        .map(a => a.join("."))
...
const props = lparen.then(prop.sepBy(spaces)).skip(rparen)
    ...
    ;
const tag1 = P.seq(
    ident.map((tag) => ({ tag, children: [] })),
    props.map((props) => ({ props: R.fromPairs(props === "" ? [] : props) })),
).map((resArr) => ({ ...resArr[0], ...resArr[1] }));
...
const compose = (x, y) => {
    ...
    return [{ ...x, children: Array.isArray(y) ? y : [y] }];
}
const fold = (x, y) =>
...
function BINARY_RIGHT(operatorsParser, nextParser) {
    const parser = P.lazy(() =>
        nextParser.chain(next =>
            P.seq(operatorsParser.skip(comment), P.of(next).skip(comment), parser.skip(comment)).or(P.of(next))
        )
    );
...
const RtmlSpec = comment.then(RtmlEquation.skip(whitespace)).many().skip(P.eof)
    .map((equationArr) =>
        equationArr.reduce((acc, obj) => ({ ...acc, ...obj })))
    ;
///////////////////////////////////////////////////////////////////////
