...
const prop = P.seq(
    ident.skip(equal),
    string // TODO: add other types (JSON, numbers, etc)
);
...
const tag = P.alt(tag1, tag2)
    .map(tag => (["TAG", { ...tag, createReact: createReact }]))
    .skip(spaces);
...
function elemComposer(acc, val) {
    ...
    const merged = {
        ...accTag,
        children: [...accTag.children, val[1]]
    }
...
function BINARY_RIGHT(operatorsParser, nextParser) {
    let parser = P.lazy(() =>
        nextParser.chain(next =>
            P.seq(operatorsParser, P.of(next), parser)
                .map(x => ([x[0], x[1]]))
                .or(P.of(next))
        )
    );
