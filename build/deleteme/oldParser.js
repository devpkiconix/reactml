...
function RtmlParser(indent) {
    return P.createLanguage({
        ...
        commaSeparatedNodes2: r => r.lparen.then(r.tag.sepBy(r.comma2).skip(r.rparen))
        ...
        arrowSeparatedNodes: r => r.node.skip(whitespace).sepBy(r.arrow).skip(whitespace)
            ...
            .map(x => [x]),
        arrowSeparatedNodes2: r => r.lparen.then(r.tag.sepBy(r.arrow2).skip(r.rparen))
            .map(arr => ramda_1.flatten(arr))
            .map(arr => arr.reduceRight((acc, tag) => {
            return (typeof tag === "string")
                ?
         ...
         */
        _spec: r => r._sentence.sepBy(r.semicolon).skip(r.End),
        _sentence: r => P.alt(r._commaSepNodes, r._node).skip(r.semicolon),
        _node: r => P.alt(r._commaSepNodes, r._arrowSepNodes, 
        // r._groupedCommaSepNodes,
        // r._groupedArrowSepNodes,
        r.tag, r.Content),
        _node2: r => P.alt(r._maybeArrowSepNodes, r._maybeCommaSepNodes, r.tag, r.Content),
        _groupedCommaSepNodes: r => P.seq(r.lparen, r._commaSepNodes, r.rparen),
        _groupedArrowSepNodes: r => P.seq(r.lparen, r._arrowSepNodes, r.rparen),
        _commaSepNodes: r => P.seq(r.node.skip(whitespace), P.seq(r.comma, r.node).many()),
        _maybeCommaSepNodes: r => P.alt(r._commaSepNodes, r.lparen.then(r._commaSepNodes).skip(r.rparen)),
        _arrowSepNodes: r => P.seq(r.node.skip(whitespace), P.seq(r.arrow, r.node).many()),
        _maybeArrowSepNodes: r => P.alt(r._arrowSepNodes, r.lparen.then(r._arrowSepNodes)
            .skip(r.rparen)),
        // P.alt(
        //     P.seq(
        //         r.lparen.then(
        //             r.node.skip(whitespace)),
        //         P.seq(r.arrow, r.node).many()
        //             .skip(r.rparen),
        //     ),
        //     P.seq(
        //         r.node.skip(whitespace),
...
exports.parseV2 = (input) => {
    const myParser = RtmlParser(0);
    return myParser._spec.tryParse(input.trim());
};
// tslint:disable-next-line:no-any
exports.parseV2F = (spec) => fluture_1.default((rej, resolve) => {
    try {
        resolve(exports.parseV2(spec));
    }
    catch (err) {
        rej(err);
    }
...
//# sourceMappingURL=oldParser.js.map