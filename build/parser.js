...
function RtmlParser(indent) {
    return P.createLanguage({
        ...
        prop: r => P.alt(P.seq(r.ident.skip(r.equal), P.alt(r.mappedPropId, r.mappedDomFuncId, r.mappedFuncId, r.value))).desc("prop"),
        ...
        commaSeparatedNodes: r => r.lparen.then(r.csn).skip(r.rparen)
            .or(r.csn)
        ...
        arrowSeparatedNodes: r => r.lparen.then(r.asn).skip(r.rparen)
            .or(r.asn)
            .map(arr => ramda_1.flatten(arr))
        ...
        node: r => P.alt(P.seq(r.tag, P.alt(whitespace.skip(r.comma).skip(whitespace).then(r.node), 
        // r.commaSeparatedNodes,
        P.seq(r.tag.skip(whitespace).skip(r.arrow).skip(whitespace), r.node))), r.Content),
        /**
         ...
         */
        Node3: (r) => P.alt(r.List, r.Composition, r.tag, r.Content.map(content => ({ tag: "Content", props: { content } }))),
        ...
        Composition: (r) => r.Node3.sepBy(r.arrow)
            ...
            return (typeof tag === "string")
                ?
                    { tag: "span", children: [tag] }
                :
                    (Object.assign({}, tag, { children: Array.isArray(acc) ? acc : [(Object.assign({}, acc))] }));
        }, {}))
            .map(compositeNode)
        // .map(x => [x])
        ,
        Equation3: r => P.seq(r.name.skip(spaces.then(r.equal).skip(spaces)), r.Node3).map(([name, def]) => ({ [name]: def })),
